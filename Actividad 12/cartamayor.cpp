...
// Referente a bajara
list<Carta> baraja;
void agregarCarta(int n, Palos p, Colores c);
...
int main(){
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if(cartaJug1.numero == 1)
            cartaJug1.numero += 13;
        if (cartaJug2.numero == 1)
            cartaJug2.numero +=13;
...
void quitarCarta(int n, Palos p, Colores c){
...
void barajear(){
    ...
    for(int i=0; i<100; i++) {
        ...
        if (pAle==Corazones||pAle==Diamantes)
            cAle = Rojo;
        else
            cAle = Negro;
       
        quitarCarta(nAle, pAle, cAle);
        agregarCarta(nAle, pAle, cAle);
    }
}
